...
func refresh_data(id):
	...
	for drop in drop_list.keys():
		var kind = drop[0]
		var kind_id = drop.erase(0, 1)
		match kind:
			"i":
				dropIdArray.append(drop)
				var item_data = item_list["item" + String(kind_id)]
				$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(item_data["name"])
			"w":
				dropIdArray.append(drop)
				var weapon_data = weapon_list["weapon" + String(kind_id)]
				$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(weapon_data["name"])
			"a":
				dropIdArray.append(drop)
				var armor_data = armor_list["armor" + String(kind_id)]
				$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(armor_data["name"])
		$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/ChanceList.add_item(drop_list[drop])
...
func _on_EnemyStatEditorOkButton_pressed():
	...
	$StatsLabel/StatsContainer/DataContainer/FormulaList.set_item_text(statEdit, statFormula)
	save_enemy_data()
	statEdit = -1
	$StatsEdit.hide()
...
func _on_DropEditOkButton_pressed():
	...
	match id:
		0:
			dropIdArray.append("i" + String(selected_id))
			var item_data = item_list["item" + String(selected_id)]
			$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(item_data["name"])
		1:
			dropIdArray.append("w" + String(selected_id))
			var weapon_data = weapon_list["weapon" + String(selected_id)]
			$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(weapon_data["name"])
		2:
			dropIdArray.append("a" + String(selected_id))
			var armor_data = armor_list["armor" + String(selected_id)]
			$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/DropsList.add_item(armor_data["name"])
	$DropsLabel/DropsContainer/VBoxContainer/HBoxContainer/ChanceList.add_item(chance)
	$DropEdit.hide()
...
func save_enemy_data():
	var json_dictionary = get_parent().get_parent().call("read_data", "Enemy")
	var enemy_data = json_dictionary["enemy" + enemy_selected]
...
func _on_eraseEnemyEffect_pressed() -> void:
	...
	$EffectLabel/PanelContainer/VBoxContainer/HBoxContainer/EffectNames.erase_item(id)
	$EffectLabel/PanelContainer/VBoxContainer/HBoxContainer/DataType.erase_item(id)
	$EffectLabel/PanelContainer/VBoxContainer/HBoxContainer/Effectvalue1.erase_item(id)
	$EffectLabel/PanelContainer/VBoxContainer/HBoxContainer/Effectvalue2.erase_item(id)
